...
function MsgActions({ content, onEdit, onReload, isUser, visible }: {
  ...
  const btnStyle = { background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', padding:'3px 4px', borderRadius:4, display:'flex' as const }
  return (
    <div style={{ display:'flex', gap:2, opacity:visible?1:0, transition:'opacity 0.15s', alignItems:'center', pointerEvents:visible?'auto':'none' }}>
      <button onClick={copy} title="Copy" style={btnStyle}
        onMouseEnter={e=>(e.currentTarget as HTMLElement).style.color='var(--text-primary)'}
        onMouseLeave={e=>(e.currentTarget as HTMLElement).style.color='var(--text-muted)'}
      >{copied?<Check size={12} style={{color:'var(--green)'}}/>:<Copy size={12}/>}</button>
      {isUser&&onEdit&&<button onClick={onEdit} title="Edit" style={btnStyle}
        onMouseEnter={e=>(e.currentTarget as HTMLElement).style.color='var(--text-primary)'}
        onMouseLeave={e=>(e.currentTarget as HTMLElement).style.color='var(--text-muted)'}
      ><Pencil size={12}/></button>}
      {!isUser&&onReload&&<button onClick={onReload} title="Regenerate" style={btnStyle}
        onMouseEnter={e=>(e.currentTarget as HTMLElement).style.color='var(--text-primary)'}
        onMouseLeave={e=>(e.currentTarget as HTMLElement).style.color='var(--text-muted)'}
...
function MessageBubble({ msg, onEdit, onReload }: { msg: Message; onEdit?: (c: string) => void; onReload?: () => void }) {
  ...
  if (isUser) return (
    ...
    <div style={{ display:'flex', justifyContent:'flex-end', width:'100%' }}
      ...
      {/*
        maxWidth: 60% — so SHORT messages shrink to content size,
        LONG messages stop at 60% of the panel width.
        This is the correct approach: max-width, not fixed width.
      */}
...
function RAGSources({ sources }: { sources: RAGSource[] }) {
  ...
  return (
    <div style={{display:'flex',flexWrap:'wrap',gap:6,paddingLeft:2}}>
      <span style={{fontSize:11,color:'var(--text-muted)',alignSelf:'center',flexShrink:0}}>Sources:</span>
      {sources.map((s,i) => (
        <a key={i} href={s.url} target="_blank" rel="noreferrer"
          style={{display:'flex',alignItems:'center',gap:4,fontSize:11,color:'#06b6d4',background:'rgba(6,182,212,0.08)',border:'1px solid rgba(6,182,212,0.25)',borderRadius:12,padding:'3px 10px',textDecoration:'none',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',flexShrink:0}}
...
export default function ChatPanel({ onOpenTerminal }: ChatPanelProps) {
  ...
  async function send(overrideText?: string) {
    ...
    try {
      ...
      if (useWebSearch) {
        await api.streamChatWeb(text, session.id, history,
          (chunk) => {
  ...
  const placeholder = !session
    ...
    : isWebMode ? 'Web search enabled — ask anything…'
    : isChat    ? 'Ask anything… (type @web for live data)'
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',overflow:'hidden',background:'var(--bg-primary)'}}>
      ...
      {currentActiveFile ? (
        ...
        <>
          {/* Messages — 24px fixed horizontal padding */}
                ...
                <div style={{fontSize:13,lineHeight:1.8}}>
                  {isChat
                    ? <>Explain concepts, review code, or answer questions.<br/>
                       Type <code style={{fontFamily:'monospace',color:'#06b6d4',background:'rgba(6,182,212,0.1)',padding:'1px 5px',borderRadius:4}}>@web</code> for live web data.</>
          ...
          {/* Input */}
          <div style={{flexShrink:0,padding:'10px 24px 16px',background:'var(--bg-primary)'}}>
            <div style={{
              ...
              />
              {isWebMode&&(
                <div style={{display:'flex',alignItems:'center',gap:3,fontSize:11,color:'#06b6d4',flexShrink:0,marginBottom:2}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:2,flexShrink:0,marginBottom:1}}>
                ...
                <button onClick={()=>send()} disabled={!canSend}
                  style={{width:32,height:32,borderRadius:8,border:'none',background:canSend?(isWebMode?'#06b6d4':'var(--accent)'):'var(--bg-hover)',color:canSend?'white':'var(--text-muted)',cursor:canSend?'pointer':'not-allowed',display:'flex',alignItems:'center',justifyContent:'center',transition:'background 0.15s',flexShrink:0}}>
