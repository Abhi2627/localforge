...
function MessageBubble({ msg }: { msg: Message }) {
  if (msg.type === 'user') return <div style={{ display: 'flex', justifyContent: 'flex-end' }}><div className="msg-user">{msg.content}</div></div>
...
export default function ChatPanel() {
  ...
  async function generateTitle(sessionId: string, firstMessage: string) {
    try {
      const data = await api.sendChat(
        `Write a short chat title of 3 to 5 plain words with spaces for: "${firstMessage.slice(0, 100)}". Reply with ONLY the title. No bold, no asterisks, no quotes, no punctuation. Just plain words with spaces. Example: Java Interview Preparation`,
        sessionId + '-titlegentmp', []
      )
      let title = data.reply?.trim() ?? firstMessage.slice(0, 40)
      // Strip any markdown bold, italic, code, quotes the model adds
      title = title.replace(/\*\*/g, '').replace(/\*/g, '').replace(/`/g, '').replace(/^["']+|["']+$/g, '').trim()
      // If still no spaces, insert before capital letters
      if (!title.includes(' ')) title = title.replace(/([A-Z][a-z])/g, ' $1').trim()
  ...
  async function send() {
    ...
    try {
      if (isProject && firstAgent) {
        ...
        setSending(false) // streaming starts immediately — no need for thinking bubble
  ...
  const placeholder    = !session ? 'Open a chat or project…' : isChat ? 'Ask anything…' : !firstAgent ? 'Add an agent from the sidebar…' : `Instruct ${firstAgent.name}…`
        ...
        {messages.length === 0 && !session?.summary && (
          <div style={{ margin: 'auto', textAlign: 'center', color: 'var(--text-muted)' }}>
            ...
            <div style={{ fontSize: 12 }}>
              {isChat ? 'I can explain concepts, review code, or answer questions' : isProject && !firstAgent ? 'Create agents from the workspace panel' : firstAgent ? `${firstAgent.name} (${firstAgent.role}) is ready` : ''}
            </div>
          </div>
        )}
