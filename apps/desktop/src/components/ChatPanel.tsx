...
export default function ChatPanel() {
  ...
  async function send() {
    ...
    setInput('')
    setSending(true)
    addMessage(session.id, { id: nanoid(), type: 'user', content: text, timestamp: Date.now() })
    try {
      if (isProject && firstAgent) {
        await api.instruct(session.id, firstAgent.id, text)
      } else if (isChat) {
        const data = await api.sendChat(text, session.id)
        addMessage(session.id, {
          id: nanoid(), type: 'agent',
  ...
  return (
    <div style={{
      ...
      {/* Input pinned at bottom — NO top border */}
      {session?.type !== 'terminal' && (
        <div style={{
          flexShrink: 0,
          padding: '10px 16px 14px',
          background: 'var(--bg-primary)',
          // No borderTop — no dividing line
        }}>
            ...
            <div style={{
              ...
              <textarea
                ...
                style={{
                  ...
                  lineHeight: 1.6, fontFamily: 'inherit', padding: '2px 0',
