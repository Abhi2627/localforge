...
function SessionItem({ session }: { session: any }) {
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {showMenu && (
        <div ref={menuRef} style={{ position:'fixed', left:228, top: (() => { if (btnRef.current) { const rect = btnRef.current.getBoundingClientRect(); return Math.min(rect.top, window.innerHeight - 160) } return 100 })(), zIndex:200, background:'var(--bg-secondary)', border:'1px solid var(--border)', borderRadius:8, padding:4, minWidth:200, boxShadow:'0 8px 24px rgba(0,0,0,0.5)' }}>
  ...
  return (
    <div style={{ background:'var(--bg-secondary)', borderRight:'1px solid var(--border)', display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      <div style={{ flex:1, overflowY:'auto', overflowX:'hidden', paddingTop:6 }}>
        ...
        {expanded ? (
          <button
            style={{ display:'flex', alignItems:'center', gap:8, width:'100%', padding:'8px 10px', border:'none', background:'transparent', color:'var(--text-secondary)', cursor:'pointer', fontSize:13 }}
            onMouseEnter={e => (e.currentTarget as HTMLElement).style.color = 'var(--text-primary)'}
            onMouseLeave={e => (e.currentTarget as HTMLElement).style.color = 'var(--text-secondary)'}
          >
            <Puzzle size={15}/><span>Extensions</span>
          </button>
        ) : (
      ...
      {/* Bottom buttons — Terminal / Account / Settings */}
      <div style={{ borderTop:'1px solid var(--border)', padding:'4px 0' }}>
        {bottomButtons.map(({ Icon, label, action }) => (
          <button key={label} onClick={action} title={!expanded ? label : undefined}
            style={{ display:'flex', alignItems:'center', gap: expanded ? 9 : 0, justifyContent: expanded ? 'flex-start' : 'center', width:'100%', padding: expanded ? '8px 12px' : '9px 0', border:'none', background:'transparent',
              // Brighter default color so they're clearly readable
