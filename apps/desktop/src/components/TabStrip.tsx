...
export default function TabStrip() {
  ...
  // Capture tab order once on mount — never reorder after that
  const tabOrderRef = useRef<string[]>([])
  const allTabs     = getRecentTabs()

  // Add new tabs to the end, never reorder existing ones
  allTabs.forEach(t => {
    ...
    <div style={{
      ...
      {tabs.map(tab => {
        ...
        return (
          ...
          >
            <span style={{ fontSize: 12, fontWeight: isActive ? 500 : 400, color: isActive ? 'var(--accent)' : 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingRight: 14, lineHeight: 1.4 }}>
              {tab.title}
            </span>
            <span style={{ fontSize: 10, marginTop: 2, color: isActive ? 'var(--accent)' : 'var(--text-muted)', textTransform: 'capitalize' }}>
              {tab.type}
            </span>
            <button onClick={e => dismiss(tab.id, e)} title="Remove from tab strip"
              style={{ position: 'absolute', top: 6, right: 5, background: 'none', border: 'none', cursor: 'pointer', color: isActive ? 'var(--accent)' : 'var(--text-muted)', display: 'flex', padding: 2, borderRadius: 3, opacity: 0.7 }}>
              <X size={10} />
            </button>
          </div>
        )
